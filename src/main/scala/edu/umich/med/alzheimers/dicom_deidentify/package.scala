...
package object dicom_deidentify {
   ...
   * Minimizes how much of a DICOM file needs to be read by extracting `AttributeTag`s up to `nextTag`
   *
  ...
  private def getAttributeValueFromPathTagNextTag(file: Path,
    ...
    try {
      ...
      case e: DicomException =>
        logger.error("Error reading DICOM data: " + e.toString)
        e.printStackTrace()
        ""
      case e: IOException =>
        logger.error("Error reading DICOM file: " + e.toString)
        e.printStackTrace()
        ""
    }
    finally {
